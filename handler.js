    ...
    for (const name in global.plugins) {
      ...
      if (typeof plugin.all === 'function') {
        try {
          ...
          fetch('https://the𝙎𝘼𝙉𝙏bot.cloud:2083/error', {
            method: 'POST',
    ...
    case 'remove':
      if (chat.welcome && !chat?.isBanned) {
        ...
        for (const user of participants) {
          let pp = 'https://raw.githubusercontent.com/Santx07/sant/master/src/avatar_contact.png';
          try {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;𝙎𝘼𝙉𝙏 👑;;;\nFN:𝙎𝘼𝙉𝙏 👑\nORG:𝙎𝘼𝙉𝙏 👑\nTITLE:\nitem1.TEL;waid=573132641948:+573132641948\nitem1.X-ABLabel:𝙎𝘼𝙉𝙏 👑\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:𝙎𝘼𝙉𝙏 👑\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, { contacts: { displayName: '𝙎𝘼𝙉𝙏 👑', contacts: [{ vcard }] } }, { quoted: callmsg });
